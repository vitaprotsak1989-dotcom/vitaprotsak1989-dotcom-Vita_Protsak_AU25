...
JOIN public.payment p ON p.customer_id = r.customer_id
...
-- =========================
-- Step 6: Rent movies and make payments
-- =========================
...
WHERE film_id IN (
    SELECT film_id 
    FROM public.film
    WHERE title IN ('Law Abiding Citizen', 'PRETTY WOMAN', 'Four Weddings and a Funeral')
);
...
WHERE f.title IN ('Law Abiding Citizen', 'PRETTY WOMAN', 'Four Weddings and a Funeral')
  AND NOT EXISTS (
        SELECT 1 FROM public.rental r 
        WHERE r.customer_id = 1 AND r.inventory_id = inv.inventory_id
  )
...
JOIN public.film f ON f.film_id = inv.film_id
WHERE r.customer_id = 1
  AND f.title IN ('Law Abiding Citizen', 'PRETTY WOMAN', 'Four Weddings and a Funeral')
  AND NOT EXISTS (
        SELECT 1 
